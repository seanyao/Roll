...
- 验收报告里的 CI 证据现在绑的是**这张卡自己的** PR:PR 号、检查跑在哪个 sha 上、每项检查的结论、合并提交,全部落进报告的「交付时事实」小节。过去这一栏取的是仓库最近一次工作流运行,跟这张卡有没有关系纯属巧合。判定只有三种诚实状态:必需检查在合并前全绿=已核实;任一失败=红(红永不被"还在跑"或"清单可能不全"盖过去);其余一律未知并写明原因(没有交付记录/PR 未合并/合并 sha 与账本不符/检查清单读不全/必需检查缺失/合并后重跑/走的是合并队列…)。必需检查取自分支保护**与** ruleset 的并集(含"必须由某个 App 出的检查"这种钉法),读不到就判未知而不是放过;报告会写明来源,并如实说明这是分支的当前配置、GitHub 不提供历史视图。采集时间晚于合并会明确标注「事后重建」,不冒充周期内证据 (US-EVID-033)[attest]
