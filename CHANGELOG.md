...
### 自动化流水线
- `roll loop` 构建完成后可以安全等待评审或测试，下一张卡不再被同一份工作目录卡住（US-CYCLE-013 / #127-#129）

### 稳定性
- 已释放派工但目录不存在的旧状态，不会再让循环永久暂停（FIX-1521 / #117）
- backlog 行内带转义竖线时，状态不再被说明文字后半段误判（FIX-1522 / #118）

### 规则层
- 现在规则登记、模块职责地图和 I1–I12 投影检查更完整（US-RULE-010, US-RULE-011, US-RULE-012, US-RULE-013, US-RULE-014, US-RULE-015 / #120-#125）

### 文档
- `roll supervisor delivery` 的读法和数据边界已补进 README、指南与帮助（US-DELTA-021 / #126）
...
### Documentation
- Delta rig 就绪状态操作文档补齐 README_CN、帮助、指南与 active skill，并加入 stale-text guard（#106）

## v4.804.1 — 2026-08-04

### 稳定性
- 交接过的任务现在能安全继续完成，不会丢失进度（FIX-1502）[loop]
- 同一任务被重复启动时，CI 不再把正常恢复误判为失败（FIX-1509）[loop]
- 发版等待超时后，现在会安全补完已核实的旧发布（FIX-1514）[loop]

### 工程和测试
- 自托管 CI 不再在构建中途找不到 pnpm（FIX-1515）[loop]
...
### 发布
- 发版结束时打印的"下一步"提示不再写死"两个名字":本仓现在只发一个 npm 名,提示会跟着**实际的发布名数量**走 —— 一个名字时不再让你去打镜像包、也不再说"每个名字都要发"(FIX-1493)[release]

### 修复
- **`roll attest --capture-command` 不再把"被 harness 杀掉"谎报成"命令退出 1"。** 以前 capture 子进程有 120 秒超时和 4MB 输出上限,像 `roll test --full` 这样的全量测试必然被杀,而杀死被错误地记成"命令退出 1" —— 绿测被判红,终端截图静默缺失,报告里只剩一段截断的无关输出。现在:上限放宽到 30 分钟 / 64MB;命令真实的退出码原样上报;harness 自己的杀死会明确标注"这是 harness 杀的,不是命令的退出码"(超时按 128+信号 上报);失败时完整的脱敏输出落到 `evidence/capture-command.{stdout,stderr}.log`,报告里的 4000 字符截断只管展示、不再管留证(FIX-1484)[attest]

## v4.731.2 — 2026-07-31

### 安装与发布
- **Roll 现在只发一个 npm 名:`@bipo-ape/roll`。** 此前它把同一份产物发到两个名字下,但 `seanyao/roll` 与 `BIPOSVC/ape-roll` 已经是**两个独立的仓、各发各自的名字** —— 从这个仓发到另一个名字下,等于把这份代码顶着另一个项目的身份发出去。**对你的影响**:从 `@bipo-ape/roll` 装的人不受影响;从旧名字装的人,`roll update` 仍然跟着**你当初装的那个名字**升级,不会被悄悄挪到另一个包上(FIX-1493)[install]
- 顺带修掉一个会让发版永久卡住的判据:发版最后一步要求"每个发布名都带上这个版本"才把 GitHub Release 从草稿提为正式版 —— 而名单里还留着本仓不该发的那个名字,于是这一步永远过不去。现在"发布目标"和"曾经装过的名字"是两份名单,各管各的;而"每个发布名都必须带上该版本"这条保证**没有被削弱**,将来再加名字仍然照拦(FIX-1493)[release]

## v4.731.1 — 2026-07-31

### 跨 agent 结对:异构判定改看模型,不看名字

- **"换了一个 agent"现在按它实际跑的模型判,不按 agent 的名字判。** 这修掉一个一直在发生的错:如果配置里把两个 agent 条目钉在同一个模型上(本仓的 `pi` 和 `reasonix` 都是 `deepseek-v4-pro`),它们以前会被当成"不同厂商"配成一对 —— 于是同一个模型评审了自己家写的代码,却报告"已独立评审"。反过来也错:`cursor` 一个条目既能跑 anthropic 的模型也能跑 openai 的,默认还是 `auto`,把它算作"一个厂商"是虚构的。**注意这是行为变更**:`pi` 与 `reasonix` 不再互为异构,你的候选池和轮换顺序会变(US-PAIR-012 / US-PAIR-015)[pairing]
- **认不出厂商的模型绝不会被当成"换了厂商"。** 它会降到能证明的最强一档并记下原因 —— "我们分辨不出来"永远不会被报成"它们不一样"。新增 agent 或模型时要在同一次改动里补上模型前缀,否则涉及它的每次比较都会静默降一档(US-PAIR-012)[pairing]
- **新增 `effort` 配置:逐关声明评审者要隔多远。** `effort: { code: vendor, score: vendor }`,或直接用预设 `standard`(缺省)/ `light` / `off`。评审强度就是表里有几关不是 `off`,不需要另一个旋钮。缺省绝不是 `off` —— 缺失、非法值、未知预设全都回落 `standard` **并报错**,要关掉评审必须亲手写出来(US-PAIR-013)[pairing]
- **新增 `roll effort`**:按(关卡 × 实际达成的隔离档)看命中率、耗时和成本,这个取舍第一次变得可以看。样本少于 10 的格子**不给命中率**并在输出里点名 —— 三五个样本算出的比率看着像趋势但不是,而 0 样本渲染成 "0%" 就是"没人反馈问题所以没有问题"(US-PAIR-016 / US-PAIR-020)[observability]
- 派活时会把配置指定的模型**传给评审者**,而不是让它用自己 CLI 的默认值(`cursor` 的默认是 `auto`,差别可能横跨厂商)。评审者自己声称跑了什么会在事后对账,不符时告警 —— 但**绝不回头改已做出的判定**:有几个 agent 的用量解析器是刻意留空的,如果观测能推翻选型,它们就永远达不到高档隔离(US-PAIR-011 / US-PAIR-018)[pairing]

### 修复:几处"分辨不出"被当成"没问题"

- **拿不到 builder 身份时,防自评检查以前会整条跳过 —— 而且不留痕。** 也就是"我们分辨不出这活是谁干的"被报成了"这是独立评审过的"。现在直接停下并说明原因(US-PAIR-017)[pairing]
- **成本 `0` 以前同时表示"免费"和"读不出用量"。** 现在分开了:读不出就明确记"不可观测 + 原因",聚合时与真实花费分列两栏,不再混算平均。顺带修掉一个让 codex 的成本**永远是 0** 的漏洞 —— 它声明用的那个用量解析器压根没注册,查表拿到空值就直接放弃了。现在少注册一个解析器会直接编译不过(US-PAIR-014)[observability]
- **事件时间戳有四分之一是秒、四分之三是毫秒,混在同一个流里。** 按时间算出来的东西全被污染(时间跨度算出来是 1970 年)。根因是三个写入方绕过了统一的写入口、各自用了秒时钟。现在写入口统一,读取侧对两个真正拿时间跟外部时钟比较的地方做归一 —— 那两处本来就是错的,混单位的流对上秒级窗口会静默丢掉不匹配的一半(FIX-1490)[observability]
- **故事卡合并改用具名检查判定。** 以前它看的是聚合后的"CI 绿不绿",而聚合会把**被跳过**的检查算成绿 —— 仓库配置里本来就有能让整套测试被跳过的条件。发版路径去年就收紧了,故事卡这条一直没跟上。现在必须那个必需检查确实在这次提交上跑过并通过;取不到具名结论时**停下等待**,不会回落到聚合放行(FIX-1489)[delivery]
- 合并前的必需检查现在可以是**一组**而不是一个:等文档漂移闸启用后,同一次提交上"测试"和"漂移检查"必须都跑过且都通过才允许合并。少跑其中任何一个都会被拒绝,并明确告诉你缺的是哪一个;一个都没配置时同样拒绝 —— 在没有闸的前提下合并比闸红更危险(FIX-1488)[delivery]
- **评审者的评分预算改成按实测分布给。** kimi 的耗时 p90 是 197 秒,而预算是扁平的 180 秒 —— 它约四分之一的调用是被**截止时间**掐掉的,不是它出错,这也是它大部分超时的来源(重试也修不了:重试继承同一个过小的预算)。现在只给"实测 p90 超过扁平值"的 agent 开例外,其余不动 —— 刻意不是给所有人调高,宽松的全局截止会把真挂死的 agent 变成长时间停摆(FIX-1492)[pairing]

### 核实后确认没有缺陷(记录在案)

- 查过"认证失败会不会被解析成一次通过的评审" —— **不会**。评审与评分两条路径对超时、非零退出、以及"有输出但没给出结论"三种情况本来就全部拒绝。历史上 458 次认证拦截**从未变成一条评审记录**,所以过去的评审数据并不脏。为防后来有人"顺手"给结论加个默认值,把这条不变量抽成了纯函数并加了用例(FIX-1491)[pairing]
- 查过"评分关六成失败是格式解析不了" —— 属实,但**早已修好**:把 66 份留存的原始输出用今天的解析器重跑,49 份能正常解析(终端控制字符、项目符号前缀、stream-json 包装都已处理);剩下 17 份里 15 份是真的没给分,属于正确拒绝。为防这层归一化静默退化,按四种真实输出形状加了回归护栏(FIX-1492)[pairing]

## v4.730.2 — 2026-07-30

### 安装与交付
- **合并前的把关更严了**:以前判断"CI 有没有过"看的是"结论里有没有失败",于是一个**被跳过**的检查会被当成通过 —— 而仓库配置里本来就有能让整套测试被跳过的条件,等于"测试没跑也能合"。现在必须是那个必需的检查**确实在这次提交上跑过并且通过**才放行;没跑和没过会分开告诉你(前者要让它跑起来,后者要修代码)。其它非必需的检查被跳过仍然不影响合并。另外 loop 交付合并时会钉住刚验过的那次提交,验完到合并之间如果有人又推了代码,合并会被直接拒绝而不是把没验过的东西合进去(FIX-1487)[delivery]
...
### 诊断
- `roll doctor` 不再报它证明不了的事。此前它会说你的代理"连不上"并建议 `launchctl unsetenv` 清掉 —— 而代理其实是通的:那条探测依赖 macOS 根本不带的 `timeout` 命令,于是**任何**代理设置都被判成失效,照它做反而会把能用的网络环境弄坏。同时 agent 检测按配置项而不是按 agent 输出,`pi`、`agy` 这类有别名的会重复出现两行,现在每个 agent 只出现一次(FIX-1494)[diagnostics]
...
- 新增 `roll-delta-team` 技能:一份不绑定任何 agent 主机的"设计→建造→评审"操作手册,由 `roll delta` 命令驱动,换个主机也照着跑(US-DELTA-005)[delta]
- 完整 Delta 模式下的"设计者"现在是独立选派、只读运行的角色,不再是建造者的别名;角色路由解析不出来就直接停下,绝不静默退回让建造者兼职(US-DELTA-006)[delta]
- 完整 Delta 模式下的"评审者"必须亲手写出自己的验收结论(含看了哪些输入、判断理由)并附自己的产物签名;过去那种由分数字段拼装出来的报告不再算通过,评审者的会话与身份必须与建造者不同,撞车在派生之前就拦下(US-DELTA-007)[delta]
- Delta 的说法和开关收口:README、架构文档、中英指南与 `roll delta` 帮助统一讲清"Delta 团队(主机引导)"与"完整 Delta 团队"的区别、本机预设的边界、以及 host attestation 只是结构校验、不能当作"新会话/新模型真的跑过"的证据。自动循环里默认不启用完整模式,要显式打开,绝不静默转换(US-DELTA-008)[delta]
...
### 交付一轮更快、更少白跑
- 长跑的子进程不再被误杀:看门狗判断"有没有在干活"改为盯这次运行**自己的**工作目录(worktree)。过去它盯的是主检出,builder 一进独立工作区就永远看着像"卡住了",于是被系统性杀掉。现在有新提交或文件变动就续命,真的停住才终止,并给出明确的终止原因(US-CYCLE-001)[cycle]
- 每一个派出去的子 agent(设计、建造、评审、评分、结对)都套上这个看门狗,并按角色设时长上限(建造 120 分钟、评审/设计 20 分钟,可在 `agents.yaml` 里改);被终止会留下带原因的记录并计入轮次账本,不再有"没人看管跑一小时"的派生(US-CYCLE-002)[cycle]
- 过大的卡在**建卡时**就被拦住:`roll story new` / `validate` 会检查颗粒度——要有验收契约、预期证据不超过 3 条、AC 不超过 6 条、预估不超过 25 分钟、并声明风险档(low/high),不合格就不让进待办;建卡时可直接用 `--est-min` / `--risk-tier` 写入。存量老卡不追溯(US-CYCLE-005)[cycle]
- 一张卡返工超过 2 轮,说明当初拆得太大:现在会照着轮次账本里的事实自动写一份拆分建议放在卡夹旁(内容没变就不重复写),`roll loop cycle split-advice` 能列出所有待处理的建议,把"跑起来才发现拆错了"回流到设计环节(US-CYCLE-006)[cycle]
- 返工那一轮不再让新 agent 重读整个仓库:开工前先生成一份带校验和的简报(评审意见全文、改动统计、涉及文件与行号、设计契约引用);超出篇幅预算就先截断意见,并如实标出截断位置和全文路径。这份简报是**替代**原来的提示词,只会更短不会更长(US-CYCLE-007)[cycle]
- 评审成本按风险分级:卡上写 low 就一个评审者,写 high 才开并行评审小组(最多 3 人,每人独立新会话);新规则下的卡要是没有合法风险档,就在评审这一步直接报错停下,绝不默认按 low 放过。分级只能从经过检查的卡片契约里读,没有开关或环境变量能偷偷降级(US-CYCLE-008)[cycle]
- PR 不再占着关键路径:开完 PR、挂上钉住 sha 的自动合并就直接返回,合没合由 git 事实确认(分支已是主干祖先,或压缩合并的补丁已在主干),不看命令输出的字面话术;确认不了就明确推迟到下一拍重试,而不是错报完成,同一次合并被重复观察也不会把卡翻两次(US-CYCLE-009)[cycle]
- 开 PR 之前强制一次全量验证:`roll test --full` 跑整套并盖上"全量"印章,交付闸只认这种新鲜的全量证据(缺失、过期、只跑了改动部分、代码树不匹配都会被拦);日常微提交仍可只跑受影响的测试。门禁耗时按"全量/改动"分开记账(US-CYCLE-011)[cycle]
- 同一个"角色 × 模型"组合在同一张卡上连败 2 次,会产出一份换模型候选,交给异构 agent 共识裁决(一致同意才通过,有异议就上报);工具绝不自动改 `agents.yaml`,换不换仍是人决定(US-CYCLE-012)[cycle]

### 稳定性
- `roll idea` 取新卡号时会先看远端(origin/main)已有的号，而不再只看本地可能过时的待办清单，避免多台机器同时建卡撞到同一个号(过去 FIX-1272/1273/1473 都撞过)。远端连不上时会退回按本地取号并给出提示；万一选中的号刚被别处占用，会明确报错让你重来，而不是悄悄写入重复号(FIX-1481)[backlog]
- 受监督的单轮 loop 不再改动你本地的共享 `main` 分支：既不会为隔离本地领先提交而重置它，也不会把待办状态更新直接提交到本地 `main`（这两种情况过去都会弄坏主工作区、连累后续派工）。隔离只在各自的周期工作区内进行，待办状态更新则安全地同步到远端，你本地未推送的改动原样保留（FIX-1475）[loop]
- 只读角色的写隔离从"口头约定"变成"真强制,否则拒绝":沙箱型 agent(codex、reasonix)按操作系统级只读运行;非沙箱 agent 被派成只读角色时,在派生边界就明确报错停下,不再表面只读实际能写。reasonix 在没有任何可写目录的情况下也会写出完整只读沙箱配置(过去这种情况会放开写权限)(FIX-1482)[agent]
- 验收报告里的 CI 证据现在绑的是**这张卡自己的** PR —— PR 号、检查跑在哪个 sha 上、逐项结论、合并提交,全部落进报告的「交付时事实」小节;过去这一栏取的是仓库最近一次工作流运行,跟这张卡有没有关系纯属巧合。只有三种诚实状态:必需检查在合并前全绿=已核实、任一失败=红、其余一律未知并写明原因,拿不准绝不放过;事后补的报告会明确标注是事后重建 (US-EVID-033)[attest]
- 修复:名字以 `cycle-` 开头的史诗目录(比如 `cycle-efficiency`)被误当成周期运行目录整个跳过,导致该史诗下所有卡对 `roll attest` 不可见(报 story not found)、永远拿不到验收报告;现在只有卡夹内部的运行目录会被跳过(FIX-1483)[attest]
...
### 稳定性
- 浏览器交互填表(fill)现在真正驱动 React 受控表单:填进去的值不再在提交时丢失 (FIX-1449)
- 受管 MCP 进程清理在 Linux 更稳:进程刚退出、内核还没回收的瞬时竞态不再被误判成"杀不掉的泄漏";真正杀不掉的仍会大声报错、绝不放过 (FIX-1450)
- 本地隔离窗口截图与 prepare 点击走 Playwright,能正确触发 React 受控状态;并锁死"没到目标状态就不截图"的保护 (FIX-005, FIX-1451)
- 发布出去的 `roll test` 现在有回归测试盯住:装好的版本面对 Vitest 3 只用 --changed、绝不把不认识的 --affected 传给 Vitest (FIX-1452)

## v4.718.1 — 2026-07-18

### 新功能(截图证据:尽力多通道,坏机器绝不冒充"已核实")
- 物理截屏和渲染截图现在共用一份统一、带来源标注的证据收据;环境变量不再能悄悄关掉截图网关 (US-PHYSICAL-009) `[loop]`
- 声明了网页界面的卡,会尽力同时走多条截图通道(Roll 截屏窗口 + 渲染截图),每次尝试都完整记录在一份证据集里 (US-EVID-030) `[loop]`
- 交付是否通过与截图证据是否健康彻底分开:证据分四种状态(已核实/机器故障降级/目标不对/契约未声明);机器坏了绝不显示为已核实,目标不对直接拦下 (US-EVID-031) `[loop]`
- `roll capture migrate` 只在截图网关和渲染器都就绪时才开启尽力截图,否则保留原策略并讲清原因,绝不瞎猜或强行翻转;`roll capture repair` 能只重跑截图通道修复降级证据、不重开交付;截图能力是否就绪现在能在 `roll doctor`、`roll capture status`、`roll loop status --capture` 里看到 (US-EVID-032) `[loop]`

### 稳定性
- 浏览器跑完后清理 MCP 进程:macOS 拒绝重复结束进程或子进程已提前退出,不再被误判成协议错误;只有进程真的清不掉才报错 (FIX-1275) `[loop]`
- 新增 `roll worktree cleanup --dry-run|--apply`:只按 audit 裁定删可弃的 worktree,绝不因"旧"或被计数就一刀切;每次删前重新审一遍,有任何变化立即拒删 (FIX-1273) `[loop]`
- 每次提交的测试闸兼容普通 Vitest 项目,不再因附加了不认识的参数在跑测试前就退出、卡住绿色交付;跑了零个测试文件绝不算通过 (FIX-1274) `[loop]`
- loop 启动闸逐一核对 .roll 改动:只有内容哈希匹配证据清单才算已核实,其余一律保守拦下,不再有笼统路径就放行的漏洞 (FIX-1272) `[loop]`
