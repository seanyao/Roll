...
### 自动化流水线
- `roll loop` 构建完成后可以安全等待评审或测试，下一张卡不再被同一份工作目录卡住（#127-#129）

### 稳定性
- 已释放派工但目录不存在的旧状态，不会再让循环永久暂停（#117）
- backlog 行内带转义竖线时，状态不再被说明文字后半段误判（#118）

### 规则层
- 现在规则登记、模块职责地图和 I1–I12 投影检查更完整（#120-#125）

### 文档
- `roll supervisor delivery` 的读法和数据边界已补进 README、指南与帮助（#126）
