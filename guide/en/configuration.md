...
| `ROLL_LOOP_FORCE` | unset | When set to any non-empty value, `roll loop` bypasses the active-window check and the pause file. `roll loop go` sets this internally for an explicitly-scoped run; export it manually only when you want a session run to ignore quiet hours. |
