...
- **No progress without a session**: delivery advances only while an owner has an agent session open running `roll loop go`. Nothing is scheduled, so throughput is bounded by how often the owner starts a run, and a run that ends mid-backlog leaves the remainder untouched until the next one.
