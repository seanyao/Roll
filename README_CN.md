...
![roll status 跑在 Roll 自己的仓库上 —— 1137 个任务经 PR 合入，健康指标一眼可见](docs/assets/readme/roll-demo.gif)
...
`main` 上的每一个 commit 都走过上面说的同一条管线：任务 → agent → 测试 → 评审 →
PR。下面的数字读自本仓库自己的 git 历史和交付账本（2026-08-18）：

| | |
|---|---|
| **时间跨度** | 116 天（2026-04-17 → 2026-08-10） |
| **Commit** | 3,478 个，分布在 106 个活跃天（日均约 33 个） |
| **PR** | 1,506 个（#1 … #1,537） |
| **代码变更量** | 全程累计 +686,541 / −214,527 行 |
| **当前规模** | TS/JS 约 32.4 万行、1,033 个文件（源码约 14.1 万，测试约 18.3 万） |
| **工作时长** | 每日 commit 跨度中位数 20.3 小时 · 74% 的活跃天跨度 ≥12 小时 · 24% 的 commit 落在周末 |
| **Agent 开销** | 435 个已交付故事、805 轮，约 163M 输入 / 38M 输出 token，估算约 $2.8k —— 数字来自 `roll metrics` 自己的账本 |

开销那行背后的逐故事账本，就是你在自己项目上也能看到的同一个视图 —— 每个故事
的轮次、token、成本、评分和 PR：

![roll metrics —— 逐故事交付账本：轮次、token、成本、评分](docs/assets/readme/roll-metrics.png)
