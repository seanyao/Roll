# Wukong 工程方法论：基于 AI Agent 的标准化交付流
...
Wukong 是一个面向 AI Agent 的**指令与工作流管理框架**。它不发明新的方法论，而是将经典软件工程实践（Scrum、TDD、TCR、SRE）编码为 AI 可执行的标准化技能集（Skills），并通过 CLI 工具实现跨客户端的配置一致性。

本文档描述 Wukong 的三层工程闭环架构及其对应的技术实现。

命名即设计哲学：悟空拥有无限变化的能力，金箍赋予他约束与纪律，而能力本身分毫不减。Wukong 框架的核心主张正在于此——AI Agent 的能力不因约束而削弱，恰恰相反，标准化的约束让这种能力在团队规模下可组合、可传递、可审计。

---

## 1. 架构总览：三层工程闭环

Wukong 将软件交付生命周期分解为三个可独立运转、又彼此反馈的闭环。每个闭环继承一组经典方法论，并通过具体的 Skill 实现自动化执行。

```mermaid
graph TB
    subgraph "Loop A: Research & Design"
        A1["$wk-research<br/>HV 竞品与技术调研"] --> A2["$wk-design<br/>需求原子化 → INVEST Stories"]
        A2 --> A3["BACKLOG.md<br/>状态索引"]
        A3 --> A4["docs/features/<br/>验收标准 & 设计方案"]
    end

    subgraph "Loop B: Implementation & Iteration"
        B1["$wk-init<br/>工程脚手架"] --> B2["$wk-story-build<br/>TCR 驱动开发"]
        B2 --> B3["$wk-.code-review<br/>提交前自审"]
        B3 --> B4["CI / Deploy"]
        B4 --> B5["Verification Gate<br/>鲜活证据验收"]
        B5 --> B6["$wk-.changelog<br/>变更沉淀"]
        B2 -.->|高风险路径| B7["$wk-spar<br/>对抗式 TDD"]
        B7 --> B3
    end

    subgraph "Loop C: Observability & Maintenance"
        C1["$wk-sentinel<br/>随机巡检"] --> C2{"异常?"}
        C2 -->|Yes| C3["$wk-bb-debug<br/>现场取证"]
        C3 --> C4["$wk-bb-analyzer<br/>根因诊断"]
        C4 --> C5["$wk-fix-build<br/>回归修复"]
        C5 --> C1
        C2 -->|No| C1
    end
...
Wukong 通过 `wukong` CLI 工具实现配置的集中管理与原子化分发。

**2.2.1 指令集挂载 (`wukong setup`)**

首次执行时，CLI 完成两项工作：

1. **建立 Single Source of Truth**：将仓库中的全局约定（`conventions/global/`）和技能定义（`skills/`）复制到 `~/.wukong/`，作为本机唯一的配置源。
2. **按 skill 逐个软链**：为 `~/.wukong/skills/` 中每个 `wk-*` 技能单独创建符号链接，挂载到各 AI 客户端目录（`~/.claude/skills/wk-*`、`~/.gemini/skills/wk-*` 等）。用户已有的 skills 不受影响，Wukong skills 独立并存。

setup 不修改任何 AI 工具的配置文件，也不改变全局 git 配置，保证零侵入。

**2.2.2 配置同步 (`wukong sync [scope]`)**

将 `~/.wukong/` 中的内容按 scope 分发到各 AI 客户端配置路径。

- `conventions`（默认）：以 `@include` 追加模式分发约定文件 — 将 WK 约定写入 `{ai_dir}/wk.md`，并在用户主配置末尾追加一行 `@wk.md`，原有内容永远不被覆盖
...
**Git Hook（可选，`wukong hooks install`）**

安装全局 `prepare-commit-msg` 钩子，自动检测当前提交的 AI 客户端来源并标记（如 `[claude code]`、`[gemini cli]`）。这是侵入性操作（修改全局 git 配置），需用户显式执行，安装前会展示当前配置并要求确认。

**2.2.3 项目级配置 (`wukong init`)**

为具体项目生成约定文件时，采用 **Global + Template 的合并策略**：

```
最终 AGENTS.md = Global AGENTS.md (组织级约束)
                + Template AGENTS.md (项目类型约束)
```

四种项目模板（`fullstack`、`frontend-only`、`backend-service`、`cli`）预定义了各自的架构约束、技术栈选型和测试要求。全局约束对所有项目生效，模板约束仅对对应类型生效。
...
组织级 (Global)    ← 代码规范、Git 纪律、TCR 流程、测试标准
  ↓ 合并
项目类型 (Template) ← 架构模式 (DDD/Clean Arch)、技术栈、目录结构
  ↓ 注入
...
| 经典方法论 | Wukong 中的实现 |
|-----------|------------|
| HCD (以人为本设计) | `$wk-research`：调研先行，数据驱动设计决策 |
| BDD (行为驱动开发) | `$wk-design`：需求以验收标准（AC）形式定义 |
| Scrum Backlog | `BACKLOG.md` + `docs/features/`：两层索引结构 |
| INVEST 原则 | Story 拆分的强制约束 |

### 3.2 调研驱动设计：`$wk-research`

在需求定义之前引入结构化调研，避免"拍脑袋设计"。`$wk-research` 实现 **HV 分析法（Horizontal-Vertical Analysis）**：
...
> **场景**：TaskFlow 计划新增"组织级权限管理"模块。动笔前，`$wk-research` 先对"B2B SaaS 权限模型"做 HV 分析——纵轴追溯 ACL → RBAC → ABAC → ReBAC 的演化脉络，横轴对比 Linear、Notion、GitHub 的权限实现方式。
>
> 交叉洞察揭示：2023 年后主流产品均在 RBAC 基础上叠加了资源级细粒度控制，纯 RBAC 已成交付下限。这个结论直接影响了后续 `$wk-design` 的 Story 拆分粒度，避免了一个"做完才发现设计不够用"的返工。
...
> **场景**：产品经理的原始需求是"管理员能看到所有人的操作记录"。`$wk-design` 将其拆解为三个独立 Story：US-007（写入审计事件）、US-008（审计列表 UI，支持过滤）、US-009（审计数据导出 CSV）。
>
...
> **设计理念 — Markdown 即代码**：在 Wukong 中，`BACKLOG.md` 和 `docs/features/` 不是开发完成后生成的文档产物——它们是驱动开发的输入。Story 没有对应 Markdown 文件就等于不存在；Story 的 Verification Gate 证据未提交，就视为未完成。文件系统是唯一的数据源，不需要同步额外的项目管理工具。
...
| TCR (Test && Commit ∥ Revert) | `$wk-story-build`：测试通过即提交，失败即回滚 |
| DevOps / CI-CD | 客观仲裁层：CI 作为"可交付"的最终裁定者；分钟级反馈回路压缩问题发现成本 |
| 防御性编程 | `$wk-spar`：对抗式 TDD 覆盖高风险路径 |

### 4.2 工程脚手架：`$wk-init`

标准化项目的目录结构与 CI 配置，消除"从零搭建"的随机性。

`$wk-init` 采用 **Global + Template 合并策略**（见 2.2.3），不同项目类型的差异主要体现在**交互层**的形态：前端项目有组件树和路由层，CLI 项目有命令注册和交互式输入，纯后端服务则只有 API 接口层。以下分模板展示各自的完整结构：
...
├── BACKLOG.md                       # 任务索引
├── AGENTS.md                        # 工程约束
├── docs/features/                   # Story 详情 & 设计方案
│
├── src/
│   ├── domains/                     # DDD 领域核心（实体、值对象、领域服务）
│   ├── app/                         # 应用服务层（用例编排）
│   └── lib/                         # 跨层共享工具
│
├── components/                      # UI 组件层（React + shadcn/ui）
│   ├── ui/                          # shadcn-generated，不可手动编辑
│   └── [feature]/                   # 业务组件（按功能分组）
│
├── app/  (或 pages/)                # 路由与页面层（Next.js App Router）
│   ├── (routes)/                    # 页面路由
│   └── api/                         # API 路由 / BFF 层
│
├── tests/
...
### 4.3 TCR 驱动开发：`$wk-story-build`

这是 Wukong 的核心执行单元。其工程意义在于：**不依赖 AI 的自述来判断代码正确性，而是以自动化测试的通过状态作为提交的唯一准则**。
...
**完整交付管线**——`$wk-story-build` 不止于本地测试通过。它要求完成端到端的交付链：
...
CI/CD 不是 Wukong 的"附加功能"，而是整个 Loop B 的**客观仲裁层**。TCR 在本地通过只是必要条件，不是充分条件——本地环境有隐性依赖、有未提交的状态、有只存在于开发机的配置。CI 在干净的、确定性的环境中对同一套代码重新执行，是"可交付"的最终裁定者。
...
Wukong 的 TCR + CI 组合将反馈窗口压缩到**分钟级**：
...
`$wk-init` 生成的 CI 配置包含两条相互独立的管线，对应不同的触发场景：
...
在鉴权、支付、数据完整性等高风险路径上，标准 TDD 的覆盖度不够——测试和实现由同一个 Agent 编写，存在认知盲区。`$wk-spar` 引入对抗机制：
...
自动触发信号：检测到 Story 涉及认证/授权、支付/计费、数据完整性校验、复杂状态机、或历史高 Bug 模块时，`$wk-story-build` 自动将该 Action 路由到 `$wk-spar`。

> **场景**：US-010（组织成员权限变更）触发 `$wk-spar` 自动路由。
>
...
- **`$wk-.changelog`**：自动从 BACKLOG.md 中提取已完成的 Story，过滤内部技术细节，生成面向用户的变更日志。
...
| 经典方法论 | Wukong 中的实现 |
|-----------|------------|
| SRE (站点可靠性工程) | `$wk-sentinel`：基于采样的自动化巡检 |
| 混沌工程 | 随机化巡检策略，模拟不可预测的检查模式 |
| 数字取证 | `$wk-bb-debug`：自动化现场证据采集 |
| 根因分析 (RCA) | `$wk-bb-analyzer`：结构化诊断报告 |

### 5.2 随机巡检：`$wk-sentinel`

`$wk-sentinel` 实现对已交付功能的持续性健康检查，核心设计思路是**用概率采样代替全量回归**，在成本与覆盖率之间取得平衡。
...
**热点检测**——自适应权重：频繁失败的 Story 自动提升采样权重。巡检发现的问题自动创建 `FIX-XXX` Backlog 条目，交由 `$wk-fix-build` 处理。

巡检通过 GitHub Actions 的 `cron` 调度运行，实现无人值守的持续监控。

> **场景**：TaskFlow v1.3 上线后，`$wk-sentinel` 以 Intensive 策略运行。第 2 次采样时，US-007（审计写入）对应的巡检用例失败——审计事件存在但 `timestamp` 字段为空。单次失败未触发告警。
>
> 第 4 次采样同一用例连续第 3 次失败，阈值触发，自动创建 `FIX-012: 审计事件时间戳为空` 并写入 Backlog，US-007 的采样权重自动提升至下一级。

### 5.3 自动化取证：`$wk-bb-debug`
...
### 5.4 根因诊断：`$wk-bb-analyzer`

消费 `$wk-bb-debug` 产出的诊断 JSON，按多维度进行结构化分析（内容状态、网络失败、DOM 渲染异常、性能瓶颈），输出诊断结论和修复建议。

> **场景（接上）**：`$wk-bb-debug` 对审计列表页面取证，Network 维度捕获到 `GET /api/audit` 返回 200 但 `timestamp` 字段值为 `null`；Console 维度同时出现 `[warn] AuditEvent serializer: missing timestamp`。
>
> `$wk-bb-analyzer` 消费诊断 JSON，定位根因：v1.3 的 ORM 升级引入了字段别名变更，序列化层未同步，导致 `created_at` → `timestamp` 的映射断裂。修复方向明确，交 `$wk-fix-build` 处理。

### 5.5 回归修复：`$wk-fix-build`

执行单一问题的修复，比 `$wk-story-build` 更轻量，但保持相同的质量要求：
...
> **场景（接上）**：`$wk-fix-build` 执行 FIX-012，修复范围严格限定为序列化层字段映射，同时添加回归测试（断言 `timestamp` 非空且为 ISO 8601 格式）。
>
> 1 个 commit，CI GREEN，部署后 Verification Gate 确认审计列表时间戳恢复正常，FIX-012 关闭。`$wk-sentinel` 下轮采样通过，US-007 采样权重回落。

---

## 6. 工程基线：Engineering Common Sense

Wukong 定义了 8 条贯穿所有闭环的非协商工程基线。这些不是"最佳实践建议"，而是每个 Story 在 Test Design Review 阶段的强制检查项：
...
除三个闭环中的主动 Skill 外，Wukong 包含一组被动触发的支撑技能：

| Skill | 触发时机 | 作用 |
|-------|---------|------|
| `$wk-.echo` | 用户输入模糊或矛盾时 | 重述意图、消除歧义后再执行，避免在错误理解上浪费算力 |
| `$wk-.code-review` | 每个 TCR micro-step 完成后 | 多维度自审（安全、可维护性、性能、范围），🔴 Critical 阻塞提交 |
| `$wk-.qa-cover` | Test Design Review 阶段 | 定义测试金字塔（Unit > E2E > Visual > Smoke）和覆盖率门禁 |
| `$wk-.changelog` | 成功部署后 | 从 BACKLOG 提取完成项，生成用户可读的变更日志 |

---

## 8. 与经典方法论的关系

Wukong 不是一个新方法论。它是将已被验证的工程实践编码为 AI Agent 可理解、可执行的标准化指令。
...
graph LR
    ...
    subgraph "Wukong 实现"
        R["$wk-research<br/>HV 分析"]
        D["$wk-design<br/>INVEST Stories"]
        SB["$wk-story-build<br/>TCR Micro-steps"]
        SP["$wk-spar<br/>对抗式 TDD"]
        SE["$wk-sentinel<br/>随机巡检"]
        BB["$wk-bb-debug<br/>自动取证"]
    end
...
关键差异在于执行主体的转换：这些方法论原本依赖工程师的纪律性（人会疲倦、会走捷径），Wukong 将它们固化为 AI Agent 的指令约束——Agent 不会"这次先跳过测试"，因为 Skill 定义中不存在这个分支。
...
- 跨 AI 客户端的配置一致性管理（`wukong` CLI）
- TCR 微步提交 + 验证门禁的质量保障机制
- Git Hook 实现的多 Agent 审计追踪

**当前局限**：

- **多 Agent 协调成本**：`$wk-story-build` 会根据 Action 的依赖关系判断是否启动并行子 Agent，但跨 Agent 的状态同步与冲突处理目前依赖约定而非强制协议，在高并发场景下仍有协调开销。
- **框架耦合**：技能定义以 Markdown 格式编写，依赖 AI 客户端对自然语言指令的理解能力——不同模型的执行精度存在差异。
- **巡检覆盖率**：`$wk-sentinel` 的采样策略有效降低了成本，但不等同于全量回归测试的覆盖保障。
...
| Skill | 阶段 | 输入 | 输出 |
|-------|------|------|------|
| `$wk-research` | 调研 | 研究主题 | Markdown/PDF 调研报告 |
| `$wk-design` | 设计 | 需求描述 | BACKLOG.md + docs/features/ |
| `$wk-init` | 初始化 | 项目名 | 标准化目录结构 + CI 配置 |
| `$wk-story-build` | 实现 | Story ID | 已部署代码 + 验证证据 |
| `$wk-roll-build` | 快速实现 | 一句话需求 | 自动拆分 → 交付 |
| `$wk-spar` | 防御性实现 | 功能描述 | 攻防测试套件 + 实现代码 |
| `$wk-fix-build` | 修复 | Fix ID | 修复代码 + 回归测试 |
| `$wk-sentinel` | 巡检 | 巡检策略 | 健康报告 / FIX 条目 |
| `$wk-bb-debug` | 调试 | URL | 诊断 JSON + 截图 |
| `$wk-bb-analyzer` | 诊断 | 诊断 JSON | 根因分析 + 修复建议 |
| `$wk-fetch` | 情报 | URL / 关键词 | 网页抓取、搜索、爬取结果 |
| `$wk-probe` | 监控 | 目标地址 | 节点发现、健康检查报告 |

## 附录 B：CLI 命令速查

| 命令 | 作用 |
|------|------|
| `wukong setup` | 首次初始化 `~/.wukong/`，挂载技能（零侵入） |
| `wukong sync conventions` | 可选：以 @include 模式追加 WK 约定（不覆盖现有文件） |
| `wukong sync skills` | 刷新 skills 并修复 per-skill symlink |
| `wukong sync all` | conventions + skills 一并执行 |
| `wukong hooks install` | 可选：安装全局 git hook（需用户确认） |
| `wukong init [type]` | 在当前目录生成约定文件（合并 Global + Template） |
| `wukong reset` | 从仓库源重置 `~/.wukong/`，然后同步 |
| `wukong status` | 显示当前配置状态、同步状态、技能链接 |
