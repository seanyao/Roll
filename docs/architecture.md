...
当前用户入口是 CLI-first：`roll init`、`roll supervisor`、`roll supervisor live`、`roll supervisor delivery`（只读统一功能交付视图）、`roll loop`、`roll status`、`roll loop cycle` 与按 Story 收口的 `roll attest` 验收 Review Page。浏览器/TUI 版 Supervisor Live Console 是下一阶段工作，不作为当前产品面承诺。安装方式：`npm install -g @seanyao/roll`。
