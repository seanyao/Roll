...
export const v3Catalog: Catalog = {
  ...
  "delta.rigs.help": {
    en: "\n  roll delta rigs [--refresh]\n    Read the local exact-model readiness snapshot; --refresh runs bounded local probes and publishes only a complete snapshot.\n",
    zh: "\n  roll delta rigs [--refresh]\n    读取本机指定模型就绪快照；--refresh 执行有界的本地探测，且只发布完整快照。\n",
  },
  "delta.rigs.title_empty": {
    en: "Delta rig readiness · no configured candidates",
    zh: "Delta 模型可派性 · 未配置候选",
  },
  "delta.rigs.title_observed": {
    en: "Delta rig readiness · observed at %s",
    zh: "Delta 模型可派性 · 观测于 %s",
  },
  "delta.rigs.title_cache": {
    en: "Delta rig readiness · %s",
    zh: "Delta 模型可派性 · %s",
  },
  "delta.rigs.no_candidates": {
    en: "No configured Delta rig candidates. Add local presets and exact rig mappings before refreshing.",
    zh: "没有已配置的 Delta rig 候选。请先添加本机 preset 和精确 rig 映射，再刷新。",
  },
  "delta.rigs.disclaimer": {
    en: "Note: a passing probe only shows that this exact model answered now. It does not prove a future long task, delivery, host-session freshness, or final role assignment; pins, exclusions, tags, cost caps, and role diversity still decide dispatch.",
    zh: "说明：探测通过只表示该指定模型刚刚可响应；它不证明后续长任务、交付、宿主会话新鲜度或最终角色分配。派工仍须独立遵守 pin、排除规则、标签、成本上限和角色多样性规则。",
  },
  "delta.rigs.group.ready": {
    en: "Ready candidates",
    zh: "可作为候选",
  },
  "delta.rigs.group.blocked": {
    en: "Blocked candidates",
    zh: "不可用候选",
  },
  "delta.rigs.group.unknown": {
  ...
  "delta.rigs.role.designer": { en: "designer", zh: "设计" },
  "delta.rigs.role.builder": { en: "builder", zh: "构建" },
  "delta.rigs.role.evaluator": { en: "evaluator", zh: "评估" },
  "delta.rigs.probe": { en: "Probe", zh: "探测" },
  "delta.rigs.observed": { en: "Observed", zh: "观测时间" },
  "delta.rigs.reason": { en: "Reason", zh: "原因" },
  "delta.rigs.state.ready": { en: "Ready", zh: "通过" },
  "delta.rigs.state.blocked": { en: "Blocked", zh: "不可用" },
  "delta.rigs.state.unknown": { en: "Unknown", zh: "未知" },
  "delta.rigs.cache.missing": {
    en: "No saved readiness observation is available; run --refresh for a local probe.",
    zh: "缓存中没有就绪观测；运行 --refresh 执行本机探测。",
  },
  "delta.rigs.cache.stale": {
    en: "The saved observation is stale (%s old); it is shown as unknown until refreshed.",
    zh: "保存的观测已过期（%s 前）；刷新前将显示为未知。",
  },
  "delta.rigs.cache.incompatible": {
    en: "The saved observation no longer matches the configured candidates; it is shown as unknown until refreshed.",
    zh: "保存的观测与当前配置候选不匹配；刷新前将显示为未知。",
  },
  "delta.rigs.reason.probe_passed": { en: "the exact model returned the minimal token", zh: "指定模型最小调用成功" },
  "delta.rigs.reason.adapter_missing": { en: "adapter executable not found; install it and refresh", zh: "未找到适配器可执行文件；安装后刷新" },
  "delta.rigs.reason.adapter_model_selection_unsupported": { en: "no safe exact-model probe is documented; it was not executed", zh: "没有已验证的安全指定模型探测方式；未执行" },
  "delta.rigs.reason.auth_required": { en: "authentication is required; sign in and refresh", zh: "需要认证；登录后刷新" },
  "delta.rigs.reason.quota_exhausted": { en: "quota or credits are unavailable; restore access and refresh", zh: "配额或额度不可用；恢复访问后刷新" },
  "delta.rigs.reason.rate_limited": { en: "provider rate limit; wait and refresh", zh: "触发服务商限流；稍后刷新" },
  "delta.rigs.reason.network_unreachable": { en: "network or VPN is unavailable; restore it and refresh", zh: "网络或 VPN 不可用；恢复后刷新" },
  "delta.rigs.reason.model_rejected": { en: "the exact mapped model was rejected; correct the mapping and refresh", zh: "指定映射模型被拒绝；修正映射后刷新" },
  "delta.rigs.reason.probe_timeout": { en: "probe timed out; retry after checking the adapter", zh: "探测超时；检查适配器后重试" },
  "delta.rigs.reason.probe_failed": { en: "probe failed without a classified diagnostic; check the adapter and refresh", zh: "探测失败且没有可分类诊断；检查适配器后刷新" },
  "delta.rigs.reason.probe_output_unverified": { en: "probe output did not verify the fixed token; refresh after checking the adapter", zh: "探测输出未能验证固定令牌；检查适配器后刷新" },
  "delta.rigs.reason.cache_missing": { en: "no saved readiness observation; run --refresh", zh: "没有保存的就绪观测；运行 --refresh" },
  "delta.rigs.reason.cache_stale": { en: "saved observation is stale; run --refresh", zh: "保存的观测已过期；运行 --refresh" },
  "delta.rigs.reason.cache_incompatible": { en: "saved observation is incompatible with current configuration; run --refresh", zh: "保存的观测与当前配置不兼容；运行 --refresh" },
  "delta.rigs.error": {
    en: "roll delta rigs: %s",
    zh: "roll delta rigs：%s",
  },
  "delta.error.unknown_subcommand": {
    en: "roll delta: unknown subcommand '%s'. Try: prepare, validate, preflight, conclude, recover-held, rigs, status, help",
    zh: "roll delta：未知子命令 '%s'。可用：prepare、validate、preflight、conclude、recover-held、rigs、status、help",
  },
  ...
  "delta.error.continuation_not_available": {
    en: "roll delta prepare: no reservation on story %s was explicitly redelegated to '%s'; --continuation-run only takes over a task that names this exact successor",
  ...
  "delta.error.continuation_adoption_failed": {
    en: "roll delta prepare: the reservation redelegated to '%s' on story %s changed while taking over; nothing was claimed and no events were written",
    zh: "roll delta prepare：接手故事 %s 上转交给 '%s' 的占用时占用发生变化；未占用、未写事件",
  },
  "delta.error.continuation_adopted": {
    en: "Delegation %s was redelegated to '%s' and run %s has taken over the story; this old task is already superseded and cannot be concluded again",
    zh: "委派 %s 已转交给 '%s'，故事已由新运行 %s 接手；旧任务已被取代，不能再次收尾",
  },
