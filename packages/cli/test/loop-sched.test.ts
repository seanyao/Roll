...
describe("loop on/off (injected launchd)", () => {
  ...
  it("off: boots out loop+dream labels and removes their plists", async () => {
    ...
    try {
      ...
      // US-LOOP-112/116: there is no guided/autonomous mode and no backend — the
      // message now states that delivery runs in the driving session.
      expect(out).toContain("delivery runs in the agent session");
    } finally {
